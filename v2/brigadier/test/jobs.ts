  ...
  describe("Container", () => {
    describe("#constructor", () => {
      ...
      it("initializes fields properly", () => {
        ...
        assert.isFalse(container.useWorkspace)
        assert.equal("/var/workspace", container.workspaceMountPath)
        assert.isFalse(container.useSource)
        assert.equal("/var/vcs", container.sourceMountPath)
