...
type SubstrateConfig struct {
	...
	// configuration.
	DefaultWorkerImagePullPolicy api.ImagePullPolicy
...
type substrate struct {
	...
	createWorkspacePVCFn func(context.Context, api.Project, api.Event) error
	createWorkerPodFn    func(context.Context, api.Project, api.Event) error
...
// NewSubstrate returns a Kubernetes-based implementation of the api.Substrate
// interface.
func NewSubstrate(
	kubeClient kubernetes.Interface,
	queueWriterFactory queue.WriterFactory,
...
func (s *substrate) CreateProject(
	...
	project.Kubernetes = &api.KubernetesDetails{
...
func (s *substrate) StartWorker(
	...
	type proj struct {
		ID         string                `json:"id"`
		Kubernetes api.KubernetesDetails `json:"kubernetes"`
	...
	eventJSON, err := json.MarshalIndent(
		struct {
			...
			Qualifiers api.Qualifiers    `json:"qualifiers"`
...
func (s *substrate) DeleteWorkerAndJobs(
	...
	if event.Worker.Status.Phase != api.WorkerPhaseCanceled &&
		event.Worker.Status.Phase != api.WorkerPhasePending {
...
func (s *substrate) createWorkerPod(
	...
	if event.Worker.Spec.Container == nil {
		event.Worker.Spec.Container = &api.ContainerSpec{}
	}
