...
func (t *tokenAuthFilter) Decorate(handle http.HandlerFunc) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if crypto.Hash("", token) == t.config.HashedSchedulerToken {
			ctx := api.ContextWithPrincipal(r.Context(), scheduler)
		...
		if crypto.Hash("", token) == t.config.HashedObserverToken {
			ctx := api.ContextWithPrincipal(r.Context(), observer)
			handle(w, r.WithContext(ctx))
			return
		}
		...
		if serviceAccount, err :=
			...
			ctx := api.ContextWithPrincipal(r.Context(), &serviceAccount)
			handle(w, r.WithContext(ctx))
			return
		}
		...
		ctx := api.ContextWithPrincipal(r.Context(), principal)
		ctx = api.ContextWithSessionID(ctx, session.ID)
		handle(w, r.WithContext(ctx))
	}
}
