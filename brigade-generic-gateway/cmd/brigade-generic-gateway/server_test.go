...
func TestNewRouterSimpleEvent(t *testing.T) {
	s := mock.New()
...
func TestNewRouterCloudEvent(t *testing.T) {
	s := mock.New()
	s.ProjectList[0].ID = "brigade-4625a05cf6914e556aa254cb2af234203744de2f"
	s.ProjectList[0].Name = "deis/empty-testbed"
	s.ProjectList[0].GenericGatewaySecret = "mysecret"
	...
	body, err := ioutil.ReadFile("./testdata/cloudevent.json")
	if err != nil {
		t.Fatal(err)
	}

	route400 := "/cloudevent/brigade-4625a05cf6914e556aa254cb2af234203744de2f_WRONG_URL/mysecret"
	res, err = http.Post(ts.URL+route400, "application/json", bytes.NewBuffer(body))
	if err != nil {
		t.Fatal(err)
	}
	...
	route401 := "/cloudevent/brigade-4625a05cf6914e556aa254cb2af234203744de2f/mysecret2"
	res, err = http.Post(ts.URL+route401, "application/json", bytes.NewBuffer(body))
	if err != nil {
		t.Fatal(err)
	}
