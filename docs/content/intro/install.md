...
A Brigade [Gateway] generates events in Brigade in response to external
triggers, for example a git push to repository. Brigade does not ship with any
gateways, which limits inbound events to those generated by the brig
CLI, such as `brigade.sh/cli:exec` (the event source and type generated from
the `brig event create` command) but compatible gateways are easy to install
as add-ons.

Peruse the [list of available gateways] once ready to install one alongside
Brigade.

[Gateway]: /topics/operators/gateways/
[list of available gateways]: /topics/operators/gateways#available-gateways
