...
git repository. They are designed to organize and coordinate running multiple
related tasks, usually in response to events. We like to think of these as
cluster-oriented shell scripts: The script just ties together a bunch of other
programs.

At the very core, a Brigade script is simply a JavaScript (or TypeScript) file
that is executed within a special cluster environment. An environment is
composed of the following things:

- A Brigade server [running in your Kubernetes cluster][quickstart]
...
gradually progressing in complexity At least starting out, all project scripts
will be defined inside of the `project.yaml` itself.
...
Next, let's trigger execution of the project script by creating an event:
...
This example executes the log in the _global scope_. While it's fine to work
in the global scope, most of the time what we want to do with Brigade is write
_event handlers_.

## Brigade Events and Event Handlers

Brigade listens for certain things to happen, such as a new push on a GitHub
repository or an image update on DockerHub. (The events that it listens for are
configured in your project under the `eventSubscriptions` section).

When Brigade observes such an event, it will load the project script and
determine if an _event handler_ matching the _event_ exists.

Using the `brig` tool introduced above, we can see how this works.

The `brig` tool triggers an `exec` event (short for _execute_) on Brigade. So
our script can intercept this event and respond to it:
...
One of the strengths of Brigade is that we can handle different events in the
same file. Brigade will only trigger the appropriate events. For example, we
can expand the example above to also provide a handler for a GitHub push event.
...
Since Brigade did not see a GitHub push event (in this case, an event from
source `brigade.sh/github` and of type `push`), it did not execute the
corresponding event handler. It only executed the handler for the event created
by `brig`, i.e. an event from source `brigade.sh/cli` and of type `exec`.
...
A typical Brigade script divides up the workload for handling events like this:

- An _event handler_ declares how an event (`brigade.sh/github:push`,
  `brigade.sh/cli:exec`) is processed
- When Brigade triggers an event that a project subscribes to, Brigade creates
  a _Worker_ per the project's configuration
- A Worker may run several _Jobs_, where each job starts a container
- A Job runs zero or more _commands_ inside of a container

And in the next section, we'll talk about how we can _group_ jobs.

In the previous section, we focused on the event handlers, which just logged
...
prototype for creating all jobs in Brigade.
...
The image is a crucial part of the Brigade ecosystem. A container is created
from this image, and it's within this container that we do the "real work". At
...
start a new `debian` pod (downloading it if necessary), execute it, and
monitor it. When the container is done executing, the run is complete.

It is worth noting that a `run()` method is _asynchronous_. This means that
when we call `run()`, it will start processing in the background. Later on, we
will see how we can take advantage of that to create pipelines.
...
paired with a list of _arguments_. The command and arguments array is added to
...
above, our command invokes the `echo` binary with the arguments supplied. For
multiple commands, a common approach is for the `command` array to consist of
one element such as `["/bin/sh"]` or `["/bin/bash"]` and then the first element
of the `arguments` array be `"-c"`, followed by entries comprising the shell
commands needed.
...
starts a Debian container and prints a message, then exits.
...
the methods we have for specifiying _how_ the jobs run, i.e. sequentially or in
parallel -- or, as we're about to see, a combination thereof.
...
- `Job.sequence()` takes an array of runnables (e.g. a job or a group of
  ...
  completed (or when any runnable fails).
- `Job.concurrent()` takes an array of runnables and runs them all
  _in parallel_. When run, all runnables are started simultaneously (subject to
  scheduling constraints). The concurrent group completes when all Runnables
  have completed.
...
sequences are run _in parallel_.
...
provide version-controlled data to our Brigade scripts. And it makes Brigade a
great tool for executing CI pipelines, deployments, packaging tasks, end-to-end
tests, and other DevOps tasks.
...
with the event, and (if there is a repository) what repository commit we should
be using.
...
`event.project.secrets` or event `event.worker.git.cloneURL` might not be the
sorts of information you want accidentally displayed. Check out the [Secrets]
guide for examples on how to safely handle project secrets in your scripts.
...
As background, the decision to make this functionality opt-in as opposed to
opt-out was based on the desire to have job results be cache-able and jobs
themselves resumable/retryable, which can currently only be accomplished when
...
terminated a short time after the job's primary container completes.

Consider an event handler that needs to run tests but also needs to provision
a backing database required by the tests. The backing database could run as a
sidecar container while the job's primary container is concerned with running
the tests.
...
Let's take a look at this example and introduce further configuration needed
for this scenario.
