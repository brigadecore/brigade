...
configured in under the `eventSubscriptions` section of its definition file
(e.g. `project.yaml`).
...
Examples of events triggered in Brigade are a new push to a GitHub repository
or an image update on DockerHub. When an event is generated for a project, that
project's worker will load the project script and determine if an _event
handler_ matching the _event_ exists.
...
> Worker image running the script. If the script fails or throws an error, the
> Worker will exit non-zero and the event will be considered to have failed.
